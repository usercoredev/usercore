...
// GetClaimsFromIDToken gets the claims from the ID Token
func GetClaimsFromIDToken(idToken string) (*IDTokenClaims, error) {
	tokenBytes := []byte(idToken)
	newToken, parseErr := jwt.ParseNoVerify(tokenBytes)
	if parseErr != nil {
		return nil, fmt.Errorf("token is not valid")
	}

	var newClaims IDTokenClaims
